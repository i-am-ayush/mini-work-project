...
public class MemberDao {
    ...
    public static boolean save(Member member) {
        try {
            PreparedStatement stmt = conn.prepareStatement("insert into member(firstName,lastName,phoneNumber,status,email,type,address,password) values(?,?,?,?,?,?,?,?)");
        ...
        try {
           // List<Member> listOfMember = new ArrayList<>();
            ...
            while (res.next()) {
                ...
                member.setStatus(Member.StatusType.valueOf(res.getString("status")));
            }
            res.close();
            stmt.close();
            return member;
